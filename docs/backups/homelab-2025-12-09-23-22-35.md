...
### 2025-12-09 - VM 102 (ubuntu-server) Resource Upgrade

**Changes:**
- **Proxmox VM Configuration** (prox-tower):
  - RAM: 20GB → 40GB (host has 78GB, plenty of headroom)
  - CPU Cores: 3 → 6 (host has 8 threads on Xeon E5-1620 v4)
  - Removed cpulimit (was 0.75, unnecessarily throttling)

- **Ubuntu VM Swap Configuration**:
  - Resized swap file: 44GB → 20GB (`/swap.img`)
  - Verified fstab entry persists across reboots

**Host Allocation After Reboot:**
| Resource | Host (prox-tower) | VM 102 |
|----------|-------------------|--------|
| RAM | ~38GB available | 40GB |
| CPU | 2 cores | 6 cores |

**Impact:**
- VM 102 now has significantly more resources for running services
- Host retains 2 cores (sufficient for Proxmox overhead)
- Swap reduced to more reasonable size relative to RAM

**VM Access:**
- IP: 192.168.2.126 (DHCP, MAC bc:24:11:73:d6:22)
- SSH: `ssh jaded@192.168.2.126`
