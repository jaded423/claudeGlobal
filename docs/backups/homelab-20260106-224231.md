...
**Last Updated:** January 6, 2026 (4TB HDD ZFS pool, media migration, Ollama moved to HDD)
...
│   ├── ZFS storage (rpool-tower SSD, ~370GB available) - converted from LVM Dec 12, 2025
│   ├── ZFS storage (media-pool HDD, 3.5TB) - 4TB Seagate ST4000DM004, added Jan 6, 2026
│   ├── Twingate Connector (systemd) - Native host service, auto-start ✅
...
│       │   ├── Ollama with 6 LLMs (port 11434) - models on HDD via NFS
...
│       │   ├── scan-and-move.sh (cron every 10min) → NFS mount on tower
│       │   ├── weekly-quality-report.py (Sundays 9AM) → email via Gmail OAuth2
│       │   └── Storage: /mnt/media-pool (NFS from prox-tower media-pool ZFS)
...
**Installed Models (Total: ~40GB, 6 models, stored on HDD via NFS):**

**Storage Location:** `/mnt/ollama/models/` (NFS mount from prox-tower `media-pool/ollama`)
**Configuration:** Systemd override sets `OLLAMA_MODELS=/mnt/ollama/models`
**Model load time:** ~13s from HDD (vs ~2s from SSD) - happens once, then runs from RAM
...
**Location**: `/media-pool/media/` on prox-tower (ZFS HDD)
**Previous Location**: `/srv/media/` on prox-book5 (migrated Jan 6, 2026)
**Last Updated**: January 6, 2026

### Serials (TV Shows)

**Path**: `/media-pool/media/Serials/` (on prox-tower)
**NFS Mount on VM 101**: `/mnt/media-pool/Serials/`
**Structure**: `Show Name/Season XX/episodes...`

**Current shows**:
```
/media-pool/media/Serials/
├── Friends/
│   ├── Season 01/
│   ├── Season 02/
│   └── ... Season 10/
...
**Path**: `/media-pool/media/Movies/` (on prox-tower)
**NFS Mount on VM 101**: `/mnt/media-pool/Movies/`
**Structure**: Standalone movies at root, franchises in parent folders

**Current collections**:
```
/media-pool/media/Movies/
...
| Date | Change |
|------|--------|
| 2026-01-06 | **4TB HDD ZFS pool & storage migration:** Installed Seagate ST4000DM004 4TB HDD on prox-tower. Created `media-pool` ZFS pool with LZ4 compression. Set up NFS exports for VM 101. Migrated media library from prox-book5 (220GB: 26GB Movies, 194GB Serials) with checksum verification. Moved Ollama models (40GB) from VM 101 SSD to HDD via NFS, freeing 37GB on SSD. Frigate remains on SSD to avoid I/O contention. |
